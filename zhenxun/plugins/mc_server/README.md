...
查询当前群绑定服务器的状态。显示在线状态、延迟、版本、在线人数、可见玩家、玩家当前在线时长、总在线时长、天气占位和 BlueMap 坐标。
...
不指定用户时查询在线时长排行，并显示每位玩家的总在线与日平均在线。指定 `@用户`、玩家名、`me`、`我` 或 `自己` 时生成个人在线情况图，并显示该范围内总在线时长。

默认范围：

- `mctime` / `mct` 直接使用时：本周目排行。
- `mctime @用户` / `mctime 玩家名` / `mctime 我`：本周个人在线图。
- `mcchart` / `mcc` 直接使用时：本日在线人数图，本日按 06:00 分界。
...
生成在线人数变化图，范围语法同 `mctime`。不写范围时默认查询 06:00 分界的本日。
...
`mctime @用户` / `mctime 玩家名` / `mctime 我` 的个人在线情况图也使用同一批 session 数据。图中的折线会按 06:00 业务日汇总在线时长，最近在线段会按查询时间范围裁剪，仍在进行中的在线段会临时截断到查询结束时间；如果 QQ 是后来才绑定的，插件会同时按当前绑定玩家查找历史 session。
