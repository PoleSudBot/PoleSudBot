...
查询当前群绑定服务器的实时状态。显示在线状态、延迟、版本、在线人数、可见玩家、玩家当前在线时长、累计在线时长、BlueMap 坐标和过去 24h 在线人数趋势。
...
不指定用户时查询在线时长排行，并显示每位玩家的总在线与日平均在线。日平均只统计查询范围内该玩家真实游玩过的 06:00 业务日，不会把没有上线的日期放进分母。指定 `@用户`、玩家名、`me`、`我` 或 `自己` 时生成个人在线情况图，并显示该范围内总在线时长和日平均在线。
...
- `mctime @用户` / `mctime 玩家名` / `mctime 我`：过去 7 天（`7d`）个人在线图。
- `mcchart` / `mcc` 直接使用时：过去 3 天（`3d`）在线人数图。

`今日`、`昨日`、`本周`、`本月` 等预设统计范围以 06:00 为日期分界。`24h`、`7d`、`1m`、`1y` 这类滚动范围从当前时刻往前数，不使用 06:00 业务日；其中 `m=30d`，`y=365d`。
...
生成在线人数变化图，范围语法同 `mctime`。不写范围时默认查询过去 3 天（`3d`）。
...
| `MC_REJOIN_SUPPRESS_SECONDS` | `90` | 玩家短时间重进时抑制进退服播报的窗口，最小按 5 秒处理。 |
| `MC_MIN_ONLINE_SESSION_SECONDS` | `180` | 低于该时长的在线段不进入统计，设为 0 可关闭过滤。 |
...
- 日志里出现 `left the game` 时会先等待 `MC_REJOIN_SUPPRESS_SECONDS`；确认离线前会再读取一次新增日志，如果玩家没有短时间重进，再按原始退出时间关闭在线段。
- 低于 `MC_MIN_ONLINE_SESSION_SECONDS` 的闭合在线段会被视为误触或闪退噪声并移出统计。
...
`mctime @用户` / `mctime 玩家名` / `mctime 我` 的个人在线情况图也使用同一批 session 数据。查询范围不超过 72h 时会按小时展示在线时长，超过 72h 时按日汇总；滚动范围按连续 24h 分桶，业务范围仍按 06:00 业务日分桶。没有在线记录的分桶会显示 0；仍在进行中的在线段会临时截断到查询结束时间。如果 QQ 是后来才绑定的，插件会同时按当前绑定玩家查找历史 session。
...
- 直接使用 `mcc` 时，默认范围是过去 3 天（`3d`）。
- `mcinfo` 下方会展示过去 24h 的同类在线人数趋势。
- `mcc` 支持 `今日`、`昨日`、`本周`、`上周`、`本月`、`上月`、`本周目`、滚动范围和日期范围；只有业务预设和日期范围按 06:00 分界。
...
- RCON 地址和密码配置正确。
- 服务端开启 RCON。
- 玩家名符合 MC Java 用户名规则：3-16 位英文、数字或下划线。
