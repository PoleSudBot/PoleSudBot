...
> 支持紧凑写法，如 `语录白丝`；`语录统计` / `语录主题` / `语录管理` 仍按原命令处理。
> **示例**: `语录` / `语录 白丝` / `语录白丝` / `语录 @小真寻`

`上传` `[图片]` `[*tag/@用户 ...*]`
> 上传图片作为语录。也可直接**回复**一张图片消息并发送 `上传`，并可同时附带手动 tag。
> 支持 `上传xxx` 这类命令头与正文不加空格的写法。

`记录` `[*tag/@用户 ...*]` (回复文本消息)
> 将回复的文本内容生成一张语录图片并保存，并可同时附带手动 tag。
> 支持 `记录aaa bbb` 这类命令头与正文不加空格的写法。
...
### 🛠️ 管理功能
`删除` [回复语录]
> **回复模式**: 回复 Bot 发送的语录图片即可将其删除。 (需为上传者或满足「删除」权限)

`删除语录` (或 `del`)
> **回复语录时**: 优先删除被回复的语录。
> **直接发送**: 删除本群**最后一条**保存的语录。 (仅限满足「删除」权限的管理人员)

`tag` / `tag all` / `alltag` (回复语录)
> 查看当前语录的 tag。`tag` 仅查看手动 tag，`tag all` / `alltag` 查看手动 tag 与自动 tag。
> 这些命令仅在回复语录图片时生效；非回复场景会静默让路，不主动提示。

`tag add` / `tag del` / `addtag` / `deltag` / `tagadd` / `tagdel` (回复语录)
> 为当前语录添加或删除**手动 tag**。支持普通文本 tag，也支持 `@用户` 作为 tag。

> 说明：手动 tag (`manual_tags`) 与 OCR / AI / 记录文本自动生成的 tag 分层存储，但查询 `语录 关键词` / `语录 @用户` 时会合并匹配。
