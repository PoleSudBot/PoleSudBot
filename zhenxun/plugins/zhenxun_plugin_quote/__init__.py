...
__plugin_meta__ = PluginMetadata(
    ...
    usage=(
        "### 核心功能\n"
        "`语录 [关键词/@用户]` - 随机发送语录，可按关键词或用户筛选\n"
        "示例：`语录`、`语录 南极`、`语录南极`、`语录 @南极`\n\n"
        "`上传 [图片] [tag/@用户 ...]` - 上传图片作为语录，也可回复图片后发送 `上传`\n"
        "示例：回复图片后发送 `上传 南极`、`上传 [图片] 南极 @南极`\n\n"
        "`记录 [tag/@用户 ...]` - 回复文本消息后生成语录图片并保存\n"
        "示例：回复消息后发送 `记录 南极`、`记录`、`记录 @南极 经典`\n\n"
        "`生成 [-s 主题ID] [-n 数量] [-o|--only]` - 预览语录图片但不入库\n"
        "`记录 [-s 主题ID] [-n 数量] [-o|--only]` - 记录时使用指定主题或连续消息\n"
        ...
        "### 说明\n"
        "`语录白丝`、`记录aaa bbb`、`上传xxx` 支持命令头与正文不加空格\n"
