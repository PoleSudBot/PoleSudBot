...
## 🃏 组卡相关
`活动组卡 [区服可选] [@用户可选] [活动ID可选] [歌曲可选] [难度可选] [模式可选]`
生成活动推荐组卡；活动未显式指定时会优先使用当前活动。
示例：`活动组卡` / `活动组卡 195 Tell Your World hd multi`

`烤森组卡 [区服可选] [@用户可选] [活动ID可选]`
生成 mysekai 组卡，不需要歌曲参数。
示例：`烤森组卡` / `烤森组卡 201`

`最强组卡 [区服可选] [@用户可选] [歌曲可选] [难度可选] [实效|倍率可选]`
生成 strongest 组卡；默认使用 `141 append`，若显式给了歌曲但没给难度，则默认 `master`。
示例：`最强组卡` / `最强组卡 Tell Your World` / `最强组卡 Tell Your World 实效`

`挑战组卡 [区服可选] [@用户可选] <角色> [歌曲可选] [难度可选]`
生成 challenge 组卡；默认歌曲 `540`、默认难度 `master`。
示例：`挑战组卡 初音未来` / `挑战组卡 初音未来 Tell Your World`
...
- 最强组卡内部仍使用 liveType，但主帮助文案默认不展开这一兼容参数。
""".strip(),
    extra={
        "author": "k1yuyu",
        "version": "3.0.0",
        ...
        "commands": [
            ...
            {"command": "sk预测 [活动ID可选]"},
            {"command": "ycx [活动ID可选]"},
            {"command": "活动组卡 [区服可选] [@用户可选] [活动ID可选] [歌曲可选] [难度可选] [模式可选]"},
            {"command": "烤森组卡 [区服可选] [@用户可选] [活动ID可选]"},
            {"command": "最强组卡 [区服可选] [@用户可选] [歌曲可选] [难度可选] [实效|倍率可选]"},
            {"command": "挑战组卡 [区服可选] [@用户可选] <角色> [歌曲可选] [难度可选]"},
            {"command": "活动剧情 <活动ID> [强制刷新]"},
            {"command": "随机四格"},
            {"command": "四格 <话数>"},
            {"command": "倍率 / 倍率计算 <a> <b> <c> <d> <e>"},
