...
class AssetProvider:
    ...
    def only_has_after_training(self, card: dict[str, object]) -> bool:
        if not self.has_after_training(card):
            return False
        return str(card.get("initialSpecialTrainingStatus", "")).strip().lower() == "done"
    ...
    def _candidate_paths(self, source: AssetSourceName, *, kind: str, **kwargs: str) -> list[str]:
        ...
        return {
            ...
            "card_thumbnail_normal": [f"startapp/thumbnail/chara/{assetbundle}_normal.png"],
            "card_thumbnail_after_training": [
    ...
    def build_candidate_urls(self, server: str, *, kind: str, assetbundle: str) -> list[str]:
        urls: list[str] = []
    ...
    async def fetch_first_content(
        ...
        *,
        ...
        for candidate in urls:
            ...
            return content
        if last_error:
            logger.warning("MoeSekai 资源抓取失败", MODULE_NAME, e=last_error)
        return None
